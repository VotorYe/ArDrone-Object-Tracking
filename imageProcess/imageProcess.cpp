    ...
    for(;;) {
        ...
        if( !paused )
        {
            ...
            if( trackObject )
            {
                ...
                err_info->y_err = (center.y - height / 2) / 240;
                int alpha = (ini_area > rect_size.width * rect_size.height) ? 1 : -1;
                err_info->z_err = sqrt(abs((float)(ini_area - rect_size.width * rect_size.height)) / 640 / 480) * alpha * 0.3;
